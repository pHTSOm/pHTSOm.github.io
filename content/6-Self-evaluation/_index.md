...
| 3 | **Proactiveness** | Good | When the new account's Bedrock on-demand quota blocked real model calls, I opened an AWS Support case myself and, rather than waiting on the outcome, built a mock summarize path so development, testing, and the demo were never blocked. The case was ultimately declined (a new-account eligibility limit, not a config issue), which made that decision to unblock myself the right one. I also went beyond the minimum scope with CI/CD, custom metrics, dashboards, and CIS compliance. |
| 4 | **Discipline** | Fair | I maintained steady working hours, followed the team's branch-protection/PR workflow, and never pushed unreviewed changes to main. That said, I fell behind on logging my worklog in real time and had to backfill it toward the end, a point to fix in future projects. |
...
* Update documentation and the worklog **as I go**, rather than reconstructing it afterward.
